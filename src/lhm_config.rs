...
pub fn git_origin(root: &Path) -> Option<String> {
    let output = crate::git::command_in(root)
...
mod tests {
    ...
    fn init_repo_with_origin(dir: &Path, url: &str) {
        let run = |args: &[&str]| {
            let status = crate::git::command_in(dir).args(args).status().expect("git invocation");
    ...
    fn test_git_origin_returns_none_when_no_remote() {
        let dir = tempfile::tempdir().unwrap();
        let status = crate::git::command_in(dir.path())
            .args(["init", "-q"])
            .status()
            .unwrap();
    ...
    fn test_is_repo_disabled_false_when_no_origin() {
        let cfg_dir = tempfile::tempdir().unwrap();
        let repo = tempfile::tempdir().unwrap();
        let status = crate::git::command_in(repo.path())
            .args(["init", "-q"])
            .status()
            .unwrap();
        assert!(status.success());
